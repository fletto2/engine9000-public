...
target_iface_t* target_targets[2];
...
{
    for (size_t i = 0; i < countof(target_targets); i++) {
...
{
  if (index >= 0 && index < (int)countof(target_targets)) {
...
{
  size_t i;
  for (i = 0; i < countof(target_targets); i++) {
